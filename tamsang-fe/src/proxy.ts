...
export async function proxy(request: NextRequest) {
  ...
  if (isProtectedRoute) {
    ...
    if (pathname.startsWith('/admin') && !user.scope?.includes('ROLE_ADMIN')) {
      return NextResponse.redirect(new URL('/', request.url)); // Forbidden
    }
    if (pathname.startsWith('/campaign-manager') && !user.scope?.includes('ROLE_ORGANIZER')) {
  ...
  if (publicRoutes.includes(pathname) && user) {
    if (pathname === '/login' || pathname === '/register') {
      ...
      let target = '/';
      if (user.scope?.includes('ROLE_ADMIN')) target = roleRoutes['ADMIN'];
      else if (user.scope?.includes('ROLE_ORGANIZER')) target = roleRoutes['ORGANIZER'];
      else target = roleRoutes['DONOR'];
